...
export class LogiXErrorListener extends antlr4.error.ErrorListener {
    ...
    syntaxError(recognizer, offendingSymbol, line, charPositionInLine, msg, e) {
        ...
        let position = lines.slice(0, line - 2).map(el => el.length).reduce((acc, val) => (acc + val), 0) + line - 1 + charPositionInLine
    ...
    applyChanges() {
        let currentValue = this.codeEditor.innerText
        let errorsToApply = this.errors.reverse();
        ...
        console.log(currentValue);
        this.codeEditor.innerHTML = currentValue;
